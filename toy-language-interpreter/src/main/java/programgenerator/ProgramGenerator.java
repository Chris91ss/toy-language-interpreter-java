...
import model.statement.semaphore.CreateSemaphoreStatement;
    ...
    public static List<Statement> getPrograms() {
        ArrayList<Statement> programs = new ArrayList<>(
                Arrays.asList(
                        ...
                        ProgramGenerator.getProgram19(),
                        ProgramGenerator.getProgram20(),
                        ProgramGenerator.getProgram21()
                ));
...
//    Procedure example
    ...
    private static Statement getProgram19() {
        // Relational MUL example with incorrect boolean condition for IF

        // MUL example with correct boolean condition for IF
        Statement declaringV1 = new VarDecStatement("v1", new IntType());
        Statement assigningV1 = new AssignmentStatement("v1", new ValueExpression(new IntValue(2)));
        Statement declaringV2 = new VarDecStatement("v2", new IntType());
        Statement assigningV2 = new AssignmentStatement("v2", new ValueExpression(new IntValue(3)));

        // Relational condition for IF: v1 != 0 (evaluates to BoolType)
        Statement ifStatement = new IfStatement(
                new RelationalExpression("!=", new VarNameExpression("v1"), new ValueExpression(new IntValue(0))),
                new PrintStatement(new MULExpression(new VarNameExpression("v1"), new VarNameExpression("v2"))),
                new PrintStatement(new VarNameExpression("v1"))
        );

        return ProgramGenerator.buildProgram(declaringV1, assigningV1, declaringV2, assigningV2, ifStatement);
    }

    private static Statement getProgram20() {
        // Wait example

        // v = 20;
        Statement declareV = new VarDecStatement("v", new IntType());
        Statement assignV = new AssignmentStatement("v", new ValueExpression(new IntValue(20)));

        // print(v);
        Statement printV = new PrintStatement(new VarNameExpression("v"));

        // wait(10);
        Statement wait10 = new WaitStatement(new ValueExpression(new IntValue(10)));

        // print(v * 10);
        Statement printVTimes10 = new PrintStatement(
                new ArithmeticExpression("*", new VarNameExpression("v"), new ValueExpression(new IntValue(10)))
        );

        return ProgramGenerator.buildProgram(declareV, assignV, printV, wait10, printVTimes10);
    }

    private static Statement getProgram21() {
        // CountSemaphore example

        // Declare variables
        Statement declareV1 = new VarDecStatement("v1", new ReferenceType(new IntType()));
        Statement declareCnt = new VarDecStatement("cnt", new IntType());
        Statement allocateV1 = new HeapAllocationStatement("v1", new ValueExpression(new IntValue(1)));

        // Create semaphore
        Statement createSemaphore = new CreateSemaphoreStatement("cnt", new ValueExpression(new IntValue(1)));

        // First thread logic
        Statement thread1 = new ForkStatement(
                new CompoundStatement(
                        new SemaphoreAcquireStatement("cnt"),
                        new CompoundStatement(
                                new HeapWriteStatement("v1", new ArithmeticExpression("*", new HeapReadExpression(new VarNameExpression("v1")), new ValueExpression(new IntValue(10)))),
                                new CompoundStatement(
                                        new PrintStatement(new HeapReadExpression(new VarNameExpression("v1"))),
                                        new SemaphoreReleaseStatement("cnt")
                                )
                        )
                )
        );
        ...
        // Second thread logic
        Statement thread2 = new ForkStatement(
                new CompoundStatement(
                        new SemaphoreAcquireStatement("cnt"),
                        new CompoundStatement(
                                new HeapWriteStatement("v1", new ArithmeticExpression("*", new HeapReadExpression(new VarNameExpression("v1")), new ValueExpression(new IntValue(10)))),
                                new CompoundStatement(
                                        new HeapWriteStatement("v1", new ArithmeticExpression("*", new HeapReadExpression(new VarNameExpression("v1")), new ValueExpression(new IntValue(2)))),
                                        new CompoundStatement(
                                                new PrintStatement(new HeapReadExpression(new VarNameExpression("v1"))),
                                                new SemaphoreReleaseStatement("cnt")
                                        )
        ...
        // Main thread logic
        Statement mainThreadLogic = new CompoundStatement(
                new SemaphoreAcquireStatement("cnt"),
                new CompoundStatement(
                        new PrintStatement(new ArithmeticExpression("-", new HeapReadExpression(new VarNameExpression("v1")), new ValueExpression(new IntValue(1)))),
                        new SemaphoreReleaseStatement("cnt")
                )
        );

        // Combine all parts into the full program
        return ProgramGenerator.buildProgram(
                declareV1, declareCnt, allocateV1, createSemaphore, thread1, thread2, mainThreadLogic
        );
    }
}
